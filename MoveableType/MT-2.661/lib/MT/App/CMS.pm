...

{
    ...
    sub list_objects {
		...
		# saul changed this line
		# note that the row highlighting doesn't work!
...
sub list_entries {
    ...
    while (my $obj = $iter->()) {
        ...
        unless ($row->{title_short}) {
            ...
            $row->{title_short} = substr($title, 0, 40) . '...';
        }
        $row->{title_short} = substr($row->{title_short}, 0, 40) . '...'
...
sub list_blogs {
    ...
    # saul added this line Thu Oct 27 11:26:06 EDT 2005
    @data = sort { $a->{name} cmp $b->{name} } @data ;
    # end saul's change
    $param{blog_loop} = \@data;
...
sub preview_entry {
    ...
    # saul rearranged the order of the lines below to put EntryMore in SuperTitle position on Fri May 27 13:14:31 EDT 2005
...
sub edit_permissions {
    ...
    # saul added the following line Tue May 17 14:27:33 EDT 2005
    @a_data = sort { $a->{author_name} cmp $b->{author_name} } @a_data ;
    # end saul's changes
    $param{author_loop} = \@a_data;
    my(@data, @o_data);
